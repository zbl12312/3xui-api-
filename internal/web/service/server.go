...
// xrayVersionsCacheTTL bounds refreshes of the Xray version picker list.
// The fork pins the list, but keeping the cache preserves the existing service
...
// GetXrayVersionsCached wraps GetXrayVersions with a TTL cache.
...
func (s *ServerService) UpdateXray(version string) error {
	...
	if !slices.Contains(versions, version) {
		return fmt.Errorf("xray version %q is not allowed", version)
	}
